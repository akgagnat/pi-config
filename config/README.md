...
Successful background completions that occur within a deterministic 250 ms window are combined into one deferred follow-up, reducing interruption noise. Failed and aborted jobs bypass that window and notify promptly without forcing an early flush of pending successes. Completion delivery remains independent of inspector visibility: the full-screen inspector runs as a focused overlay so parent follow-up turns can continue beneath it without withholding results. `subagent_wait` consumes both pending and future deferred notices for the requested ids; whichever operation wins the event-loop race owns delivery, so a result returned by `subagent_wait` is not later repeated. Completion batches and timers are in-memory only and are discarded on shutdown/reload.
